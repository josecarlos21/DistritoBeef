...
export const EventDetail: React.FC<EventDetailProps> = ({ event, onClose, onAction: _onAction }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col md:flex-row bg-background-dark/95 backdrop-blur-xl animate-in slide-in-from-bottom-5 duration-300">
      ...
      {/* Card */}
      <div className="relative w-full sm:max-w-md bg-deep rounded-t-[40px] sm:rounded-[40px] overflow-hidden shadow-[0_0_80px_rgba(0,0,0,0.9)] border-t border-white/10 sm:border flex flex-col animate-in slide-in-from-bottom-full duration-500 max-h-[92vh]">

        {/* Scrollable Container */}
        <div className="overflow-y-auto no-scrollbar flex-1 relative bg-deep">
          ...
          <div className="h-80 relative shrink-0 group">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[var(--bg)] via-[var(--bg)]/30 to-transparent"></div>

            <button
              type="button"
              onClick={() => { triggerHaptic('light'); onClose(); }}
              aria-label={t('action.close')}
              className="absolute top-4 right-4 w-10 h-10 rounded-full backdrop-blur-md text-white flex items-center justify-center border z-10 hover:bg-white/20 transition-all active:scale-95 shadow-lg bg-black/40 border-white/10"
            >
          ...
          {/* Content Body */}
          <div className="p-6 pt-2 bg-deep">

            {/* Key Info Grid */}
            <div className="grid grid-cols-1 gap-4 mb-8 bg-white/5 p-5 rounded-[32px] border border-white/5 shadow-inner">
              {/* Date */}
              <div className="flex items-start gap-3">
                <Calendar size={18} className="text-o shrink-0 mt-0.5" strokeWidth={2.5} />
                <div>
                  <p className="text-[10px] text-f uppercase font-black tracking-widest mb-0.5">{t('event.date')}</p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-[10px] text-f uppercase font-black tracking-widest mb-0.5">{t('event.time')}</p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-[10px] text-f uppercase font-black tracking-widest mb-0.5">{t('event.venue')}</p>
            ...
            <div className="mb-8">
              ...
              {event.dress && (
                <div className="mt-5 flex items-center gap-3 p-3 bg-white/[0.03] rounded-2xl border border-white/5">
            ...
            {/* Action Buttons Grid */}
            <div className="grid grid-cols-3 gap-3 pb-safe-offset-4">
              {/* Website */}
              {event.url && (
                <a
                  href={event.url}
                  ...
                  className="h-12 bg-white/5 border border-white/10 text-white rounded-2xl flex items-center justify-center gap-2 hover:bg-white/10 transition active:scale-[0.98]"
                >
              ...
              <button
                ...
                className={cx(
                  "h-12 bg-white/5 border border-white/10 text-white rounded-2xl flex items-center justify-center gap-2 hover:bg-white/10 transition active:scale-[0.98]",
            ...
            {/* Agenda Button (Sticky-ish bottom within scroll or flows normally? Let's keep it in flow but at bottom) */}
            <button
              type="button"
              onClick={() => {
              ...
              className={cx(
                "w-full h-14 mt-4 rounded-2xl flex items-center justify-center gap-3 transition-all font-black uppercase tracking-[.2em] text-[11px] shadow-bento",
                isInAgenda
            ...
            <p className="text-[9px] text-center text-white/40 uppercase tracking-widest mt-4">
              {!isAuthenticated
