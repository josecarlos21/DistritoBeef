...
import { getHour, triggerHaptic, getFullDateLabel, cx, useImageLoading } from '../utils';
...
const EventCard: React.FC<EventCardProps> = ({ 
  ...
  const { src: imageSrc, isLoaded } = useImageLoading(event.image, event.id);
  ...
  return (
    <div className="flex gap-4 relative group animate-in slide-in-from-bottom-2 duration-500">
       ...
       >
          {/* Bg */}
          <div 
            className="absolute inset-0 bg-cover bg-center opacity-50 mix-blend-overlay transition-all duration-700 group-hover:scale-105" 
            style={{ 
