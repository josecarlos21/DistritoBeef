...
import { GlassContainer, IconButton, Badge } from './UI';
        ...
        <GlassContainer strong className="overflow-hidden">
            ...
            <div className="px-6 relative -mt-12 flex flex-col items-center text-center">
                <div className="relative">
                    ...
                    {user.online && (
                        <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-[var(--ok)] border-4 border-[#14110C] rounded-full" />
                    )}
                </div>
                ...
                <div className="mt-4">
                    <h2 className="text-2xl font-black text-white font-display flex items-center justify-center gap-2">
                        {user.name}, {user.age}
                        <Shield size={16} className="text-[var(--o)]" fill="currentColor" />
                    </h2>
                    <div className="flex items-center justify-center gap-1.5 mt-1 text-[var(--s)]">
                        <MapPin size={12} />
                ...
                <div className="w-full text-left space-y-4 mb-6">
                    <div>
                        <div className="text-[9px] font-black uppercase tracking-[.2em] text-[var(--f)] mb-2">Intereses</div>
                        <div className="flex flex-wrap gap-2">
                            {tags.map(t => (
                                <span key={t} className="px-3 py-1 rounded-lg bg-white/5 border border-white/10 text-[10px] font-bold text-[var(--m)]">
                                    {t}
                                </span>
                            ))}
                        </div>
                    ...
                    <div>
                         <div className="text-[9px] font-black uppercase tracking-[.2em] text-[var(--f)] mb-2">Última vez visto</div>
                         <div className="flex items-center gap-2 p-3 rounded-xl bg-white/[0.03] border border-white/5">
                            <Music size={16} className="text-[var(--o)]" />
                            <div>
                                <div className="text-xs font-bold text-white">Blue Chairs Beach Club</div>
                                <div className="text-[9px] text-[var(--f)]">Hace 15 minutos</div>
                            </div>
                         </div>
                    </div>
                </div>
